...
export function useOnchainZap(
  ...
  const mutation = useMutation<OnchainZapResult, Error, OnchainZapArgs>({
    ...
    onSuccess: (result) => {
      // Optimistically mark the target as zapped-by-me so any UI tracking
      // user zaps updates immediately, without waiting for the relay echo.
      ...
      // If the caller opted into handling success themselves, skip the toast.
      if (onSuccess) {
        onSuccess(result);
      } else {
        toast({
